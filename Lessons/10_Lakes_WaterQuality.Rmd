...
# load(file = "/Users/ethanready/Hydrologic_Data_Analysis/Data/Raw/LAGOSdata.rda")
...
> Variable 1: sampleyear

> Variable 2: chla

> Variable 3: secchi

> Variable 4: samplemonth
...
```{r}
# Wrangle data here.
LAGOS_predictors<-left_join(LAGOSdata$state, LAGOSdata$locus, by = "state_zoneid" )%>%
  left_join(LAGOSdata$epi_nutr, by = "lagoslakeid")%>%
  mutate(sampleyear= year(sampledate), 
         season = as.factor(quarter(sampledate, fiscal_start = 12)))%>%
  select(state, sampleyear, season, chla, secchi, tn, tp, lagoslakeid)%>%
  drop_na()

levels(LAGOS_predictors$season) <- c("Winter", "Spring", "Summer", "Fall")
```
...
```{r}
nutr<-cor(LAGOS_predictors[,c(2,4:7)])
corrplot(nutr, method = "ellipse", type = "upper")
```
...
```{r}
lm.tn<-lm(tn~chla+season+sampleyear+secchi+state, LAGOS_predictors)
lm.tp<-lm(tp~chla+season+sampleyear+secchi+state, LAGOS_predictors)
